...
class TrendDetectionModel:
    def __init__(self, input_shape, num_classes=3, conv1_filters=64, conv2_filters=128, lstm_units=128, dense_units=64, dropout_rate=0.4, learning_rate=0.0005, batch_size=64):
    ...
    def build_model(self):
        """Builds the CNN-LSTM model for trend classification."""
        ...
        x = tf.keras.layers.Reshape((-1, 1))(x)  # Reshape for LSTM compatibility
    ...
    def train(self, X_train, y_train, X_test, y_test, epochs=50):
        ...
        self.model.fit(X_train, y_train, epochs=epochs, batch_size=self.batch_size, validation_data=(X_test, y_test), callbacks=[early_stopping, reduce_lr])
